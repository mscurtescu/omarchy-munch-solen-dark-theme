...
carries type. Dark inverts the relationship: straw halo becomes the page,
...
- **Solar border.** `hyprland.lua` paints active windows with a 3-stop ray
  gradient (cadmium → yellow → straw/gold) and loops Hyprland's native
  `borderangle` animation at its slowest speed, so the gradient slowly sweeps
  the window. Inactive windows sink into the rock/earth tone.
...
- **Icons / keyboard.** Yaru (`Yaru-yellow-dark` on dark, `Yaru-yellow` on
  light) plus a `keyboard.rgb` accent.
